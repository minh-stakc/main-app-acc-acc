...
void readSoil() {
  ...
  // Populate the JSON object with sensor data
  jsonDoc["soil_ph"] = ph / 100;
  jsonDoc["soil_humi"] = Humi/ 10;
  jsonDoc["soil_temp"] = Temp / 10;
  jsonDoc["nito"] = Nito;
  jsonDoc["photpho"] = Photpho;
  jsonDoc["kali"] = Kali;
  jsonDoc["EC"] = EC;
...
void cylinderWithSpeed(int direction, int speed) {
  cylinder_state = direction;
  if (direction == 1) {
    digitalWrite(CylR_PWM, HIGH);  // extend
    digitalWrite(CylL_PWM, LOW);
